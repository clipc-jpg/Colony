...
pub fn GeneralContainerPage(app_state: Signal<AppState>, comm_with_backend: Signal<FrontendCommChannel>) -> Element {
    ...
    let _clear_backend_output = use_effect(move || {
        if running_job().is_none() { backend_output.with_mut(|v| {v.clear();} ) }
    });
    ...
    rsx! {
        div {
            ...
            div {
                ...
                StopEntireProgramOverlay { comm_with_backend, is_visible: stop_entire_program_overlay_visible }
                JobObserverOverlay { is_visible: job_observer_visible, running_job, backend_output, stop_entire_program_overlay_visible, comm_with_backend }
                div {
                ...
                div {
                    ...
                    RunningContainerCard { job: running_job, job_observer_visible  }
...
pub fn ConfigureSelfCard(class: String, app_state: Signal<AppState>,
    ...
    rsx! {
        div {
            ...
            button {
                class: format!("configure-self-button primary-button {class}"),
                onclick: move |_| {
                    spawn(async move {
                        ...
                        match (webserver_port(), container_path()) {
                            (Some(port), Some(cont)) => {
                                comm_with_backend.read().send(BackendRequest::RunSingularityApp(PathBuf::from("~"), cont.clone(), "self-configurator".to_string(), vec!["--colony-interop".to_string()])).ok();
                                ...
                                app_state.set(AppState::ContainerSelfConfiguratorPage(cont.clone(), ip_address));
                            },
                            (None, Some(cont)) => {
                                println!("No webserver running!");
                                comm_with_backend.read().send(BackendRequest::StartLocalWebServer(app_state().clone(), cont.clone())).ok();
                            },
                            (port, cont) => {println!("Port: {:?}, Container path_ {:?}", &port, &cont)}
                        }
                    });
                },
...
#[component]
fn RunningContainerCard(job: Signal<Option<ContainerPageJobState>>, job_observer_visible: Signal<bool>) -> Element {
...
#[component]
fn StopEntireProgramOverlay(comm_with_backend: Signal<FrontendCommChannel> ,is_visible: Signal<bool>) -> Element {
        ...
        div {
            ...
            div {
                ...
                h3 {
                    ...
                    "Stopping all spawned processes currently requires"
                    br { }
                    "terminating the whole program. Do you want to continue?"
                }
                div {
